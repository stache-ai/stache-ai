...
- **`context=` parameter**: `PineconeVectorDBProvider` data methods (`insert`, `search`, `delete`, `delete_by_metadata`, `get_by_ids`, `get_vectors_with_embeddings`, `get_collection_info`) accept an optional keyword-only `context` parameter (request context passed through from stache-ai's pipeline). This provider ignores it.
