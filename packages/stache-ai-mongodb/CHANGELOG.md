...
- **`context=` parameter**: `MongoDBDocumentIndex` and `MongoDBNamespaceProvider` methods accept an optional keyword-only `context` parameter (request context passed through from stache-ai's pipeline). This provider ignores it.
